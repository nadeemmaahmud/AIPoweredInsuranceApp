...
import json
from .models import PremiumChatMessage
...
class ChatBotConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        room = await sync_to_async(PremiumChatMessage.objects.filter(participants=self.user).first)()

        if not room:
            room = await sync_to_async(PremiumChatMessage.objects.create)(
